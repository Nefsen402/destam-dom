...
It can be sometimes useful to ducktype a dom node to mount to as mount() serves as
the interface to basically unwrap and extract the actual dom tree an arbitrary
dom template generated by destam-dom. This can be useful when implementing
components where they want to attach listeners or extract state from the children.
